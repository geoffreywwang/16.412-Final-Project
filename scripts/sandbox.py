...
from world_gen import generate_gazebo_world
...
x0_bar = np.array([9, 1, 0, 0]) # The mean of x at timestep 0
Sigma_x0 = np.zeros((4, 4)) # Covariance matrix of x at timestep 0
Sigma_x0[0, 0] = .05
Sigma_x0[1, 1] = .05
Sigma_x0[2, 2] = .001
Sigma_x0[3, 3] = .001
Sigma_w = 0.00001*np.identity(4) # Noise covariance added to x at each timestep
xf_bar = np.array([2, 8, 0, 0]) # Final state
state_bounds = [(-40, 40), (-40, 40), (-15, 15), (-15, 15)] # Bounds for our state vector
input_bounds = [(-1.2, 1.2), (-1.2, 1.2)] # Bounds for our input vector
...
# Plan with Delta = .4
Delta = 0.01 # Set chance constraint
...
# Plan with Delta = .4
Delta = 0.1 # Set chance constraint
p3 = Planner(T, 4, 2, T_length, A, B, x0_bar, xf_bar, Sigma_x0, Sigma_w, Delta, state_bounds, input_bounds)
p3.include_trajectories(data) # Add trajectories to planner
p3.plan_with_IRA() # optimize path
x3 = p3.get_x() # get plan states
# Get objective value
print("Delta=.001 Objective: ", p1.get_objective())
print("Delta=.01 Objective: ", p2.get_objective())
...
# visualize
objs = p2.get_objects()
generate_gazebo_world([x1, x2, x3], objs)
visualize(xs=[x1, x2, x3], labels=["Delta = .001", "Delta = .01", "Delta = .1"], objs=objs, c=["r", "g", "b"])
print()
...
# # visualize
# objs = p2.get_objects()
# generate_gazebo_world([x1, x2], objs)
